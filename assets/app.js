/* =========================================
   TANDA CRM - QUOTATION SYSTEM (Simplified)
========================================= */
...
const TANDA_Notify = {
    init: function() {
        ...
        if(!document.getElementById('tanda-modal-overlay')) {
            ...
            mo.innerHTML = `
                <div class="kb-modal">
                    <h3 id="tanda-modal-title"></h3>
                    <div id="tanda-modal-msg"></div>
                    <div id="tanda-modal-actions" class="mt-4"></div>
                </div>`;
    ...
    alert: function(message, title = 'Thông báo') {
        return new Promise(resolve => {
            ...
            const actions = document.getElementById('tanda-modal-actions');
            actions.innerHTML = `<button class="kb-btn kb-btn-primary" id="tanda-btn-ok">Đã hiểu</button>`;
            overlay.classList.add('show');
            document.getElementById('tanda-btn-ok').onclick = () => { overlay.classList.remove('show'); resolve(true); };
        });
    },
    prompt: function(message, title = 'Nhập thông tin', defaultValue = '') {
        return new Promise(resolve => {
            this.init();
            const overlay = document.getElementById('tanda-modal-overlay');
            document.getElementById('tanda-modal-title').innerText = title;
            document.getElementById('tanda-modal-msg').innerHTML = `${message}<br><br>
                <input type="text" id="tanda-prompt-input" class="input-premium text-xl" value="${defaultValue}" style="text-align:center; font-weight:bold; color: #dc2626;">`;
            
            const actions = document.getElementById('tanda-modal-actions');
            actions.innerHTML = `
                <button class="kb-btn kb-btn-secondary" id="tanda-btn-cancel">Hủy</button>
                <button class="kb-btn kb-btn-primary" id="tanda-btn-ok">Xác nhận</button>
            `;
            overlay.classList.add('show');
            const input = document.getElementById('tanda-prompt-input');
            input.focus();
            ...
            document.getElementById('tanda-btn-cancel').onclick = () => { overlay.classList.remove('show'); resolve(null); };
            document.getElementById('tanda-btn-ok').onclick = () => { overlay.classList.remove('show'); resolve(input.value.trim()); };
        });
    }
};
...
const staffProfiles = {
    'admin': { fullName: 'Quản Trị Viên', email: 'admin@tanda.vn', phone: '0933 129 155', role: 'Quản lý Hệ thống' },
...
async function checkSession() {
    try {
        ...
        if (data.status === 'logged_in') {
            ...
            localStorage.setItem('tanda_full_name', data.full_name || '');
            localStorage.setItem('tanda_email', data.email || '');
            localStorage.setItem('tanda_phone', data.phone || '');
            localStorage.setItem('tanda_role', data.position || data.role || 'Phụ trách Kinh doanh');
            showApp();
        } else {
...
async function login() {
    ...
    try {
        ...
        if (data.status === 'success' || data.status === 'logged_in') {
            ...
            else TANDA_Notify.alert(data.message, "Đăng nhập thất bại");
        }
    } catch(e) {}
...
function switchTab(tabId) {
    const allTabs = ['form-tab'];
    const allBtns = ['btn-form-tab'];
...
async function fillHistoryToForm(dbId) {
    ...
    if(!item) return;
    let isOk = await TANDA_Notify.alert(`Tải lại dữ liệu của <b>${item.quote_no}</b>?<br>Form hiện tại sẽ bị ghi đè.`);
    if(!isOk) return;
...
async function saveQuoteToDB() {
    if(!safeVal('quoteNo')) return TANDA_Notify.alert("Vui lòng điền số báo giá!", "Cảnh báo");
    if(!safeVal('buyerName')) return TANDA_Notify.alert("Vui lòng điền tên khách hàng!", "Cảnh báo");
    ...
    try {
        ...
        try {
            ...
            TANDA_Notify.alert(`<b>SERVER DATABASE BÁO LỖI:</b><br><div class="mt-2 text-xs text-red-600 bg-red-50 p-3 border border-red-200 rounded max-h-40 overflow-auto text-left font-mono">${rawText || "Không có phản hồi"}</div><br><i>*Hãy chụp màn hình này gửi Team dev!</i>`, "Lỗi Hệ Thống");
            if(btn) { btn.innerHTML = "LƯU VÀO SERVER NAS"; btn.disabled = false; }
            return;
        }

        if (data.status === 'success') { 
            TANDA_Notify.toast(data.message, "success");
            loadHistoryFromDB();
        } else {
            TANDA_Notify.alert(data.message, "Lỗi từ Backend");
        }
    } catch(e) { 
        TANDA_Notify.alert("Mất kết nối hoàn toàn tới API! Chi tiết: " + e.message, "Lỗi Mạng"); 
    }
    if(btn) { btn.innerHTML = "LƯU VÀO SERVER NAS"; btn.disabled = false; }
}
...
function updateDoc() {
    try {
        ...
        if (assignedUser === window.currentUser && localStorage.getItem('tanda_full_name')) {
            profile.fullName = localStorage.getItem('tanda_full_name');
            profile.email = localStorage.getItem('tanda_email') || profile.email;
            profile.phone = localStorage.getItem('tanda_phone') || profile.phone;
            profile.role = localStorage.getItem('tanda_role') || profile.role;
        }
        ...
        if (isRetail) {
            ...
            quoteHtml = quoteHtml.replace(/Ngân hàng ACB[\s\S]*?\(CN Ông Ích Khiêm\)/gi, "Ngân hàng Techcombank");            
            quoteHtml = quoteHtml.replace(/(Chủ Tài Khoản:[\s\S]*?)CÔNG TY TNHH TM DV/gi, "$1CÔNG TY TNHH KTS");
        }
